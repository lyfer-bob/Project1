<?php include("session.php");
      include("layout.php");
      require("connect.php");
      $sql = "select * from image where name = '" . $_GET['pjname']."'";
      $sql22 = "select namecus from project where name = '" . $_GET['pjname']."'";
  ...
  <form name="sentMessage" id="contactForm" method="get"
    ...
    <div class="row">
        <div class="col-lg-12">
                  <h5 align = 'right'>
                      <?php
                      $sql11 = "select * from users where pjname = '" . $_GET['pjname']."'";
                      $rss = $conn->query($sql22);
                ...
                <span style="padding-left:1040px">

                   <?php
                    $rs2 = $conn->query($sql11);
                    while($row = $rs2->fetch_assoc())
                    {

                      echo "  <a href='editpass.php?name=".$row['name']."'class='btn btn-primary'>แก้ไขรหัสผ่าน</a>";
                    }

                     ?>


                  <h1 class="page-header"><?php echo  $_GET["pjname"]; ?>
                 ...
                 <h4> วันที่เริ่มทำ :
                      <?php
                      $st1 = "select timestart from project where name = '".$_GET['pjname']."'";
                      $rs = $conn->query($st1);
                      if($rs->num_rows > 0)
                      {
                    ...
                    <span style="padding-left:200px">วันที่สิ้นสุด :
                      <?php
                      $st1 = "select timestop from project where name = '".$_GET['pjname']."'";
                      $rs = $conn->query($st1);
                      if($rs->num_rows > 0)
                      {
                        while($row = $rs->fetch_assoc())
                 ...
                 <?php
                 $bb = "select progress from project where name = '".$_GET['pjname']."'";
                 $rs = $conn->query($bb);
                 if($rs->num_rows > 0)
                 {
                   while($row = $rs->fetch_assoc())
    ...
    <?php
              $sql = "select * from image where name = '" . $_GET['pjname']."' order by time DESC";
              $rs = $conn->query($sql);
              while ($row = $rs->fetch_assoc())
              {
                echo "<div class='row'>";
